...
def main():
    ...
    if len(sys.argv) == 2 and (sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml")):
        ...
        # 2. Flatten the nested YAML blocks into a single dictionary
        flat_args = {}
        for section in ["model", "data", "training", "logging"]:
            if section in yaml_config and yaml_config[section] is not None:
    ...
    def prepare_dataset(batch):
        ...
        if model_args.pseudo_ipa:
            input_str = clean_bambara_pseudo_ipa(input_str)
        
        if is_uroman:
            input_str = uromanize(input_str, uroman_path=uroman_path) ###### HERE
